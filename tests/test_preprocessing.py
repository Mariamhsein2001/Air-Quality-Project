...
def test_preprocessor(sample_config, sample_data):
    ...
    expected_train_size = int((1 - sample_config.splitting.test_size) * sample_data.shape[0])
    ...
    assert not (X_train.values == original_features[:len(X_train)]).all().all(), "X_train should be scaled."
    assert not (X_test.values == original_features[len(X_train):]).all().all(), "X_test should be scaled."
